...
exec > /root/setup-NirWaNa.log 2>&1
...
(echo "oke#8888"; sleep 2; echo "oke#8888") | passwd > /dev/null

# setup hostname and timezone
echo "setup hostname and timezone to asia/jakarta"
uci set system.@system[0].hostname='NirWaNa'
...
if grep -q "Raspberry Pi" /proc/cpuinfo; then
  ...
  uci set wireless.@wifi-iface[1].ssid='NirWaNa_5G'
  uci set wireless.@wifi-iface[1].encryption='none'
else
  uci set wireless.@wifi-device[0].channel='9'
  uci set wireless.@wifi-iface[0].ssid='NirWaNa'
fi
uci commit wireless
wifi reload && wifi up
